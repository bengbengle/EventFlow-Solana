...
scripts that can be used to interact with an [quest-program](https://github.com/paul-schaaf/solana-quest)


## Commands

All available commands can be found in the `package.json` file. Start by going into your quest program repo and building your program with
```
cargo build-bpf
```
...
Now, in another tab, you can start executing the scripts to interact with your quest program. There are three scripts: `setup`, `alice`, and `bob`. `setup` initializes SOL accounts as well as the necessary token accounts for Alice and Bob. `alice` executes Alice's transaction and `bob` executes Bob's transaction. Start by installing the necessary dependencies.
```
npm install
```
